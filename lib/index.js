...
const createSizes = async (file, config) => {
  ...
  const imagesToCreate = imageSizes.map(async size => {
    ...
    if(webp === 'true') images.push(await sharp(buffer).resize(width, height, {fit: sharp.fit.cover, withoutEnlargement: true}).webp({quality: imgQ}).toBuffer()
...
const getSizesToDelete = (file, config) => {
  ...
  const sizesToDelete = imageSizes.map(size => {
    ...
    /* Create the WebP image for each size if enabled */
    if(webp === 'true') sizes.push({suffix:`-${width}-${height}.webp`})
    return sizes;
  });
...
module.exports = {
  ...
  auth: {
    ...
    s3ForcePathStyle: {
      label: 's3ForcePathStyle: Whether to force path style URLs for S3 objects',
      type: 'enum',
      values: [
        'false',
        'true'
    ...
    signatureVersion: {
      label: 'signatureVersion: The signature version to sign requests with',
      type: 'enum',
      values: [
        'v4',
        'v3',
