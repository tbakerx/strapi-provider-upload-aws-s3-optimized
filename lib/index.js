...
const createSizes = async (file, config) => {
  ...
  const imagesToCreate = imageSizes.map(async size => {
    ...
    switch(file.ext) {
      case '.png': images.push(await sharp(buffer).resize(width, height).png({quality: imgQ}).toBuffer()
          .then(data => ({buffer: data, mime:file.mime, ext: file.ext, suffix:`-${width}-${height}${file.ext}`})))
        break;
      case '.jpg': images.push(await sharp(buffer).resize(width, height).jpeg({quality: imgQ}).toBuffer()
          .then(data => ({buffer: data, mime:file.mime, ext: file.ext, suffix:`-${width}-${height}${file.ext}`})))
        break;
      case '.jpeg': images.push(await sharp(buffer).resize(width, height).jpeg({quality: imgQ}).toBuffer()
          .then(data => ({buffer: data, mime:file.mime, ext: file.ext, suffix:`-${width}-${height}${file.ext}`})))
        break;
    }
    /* Create the WebP image for each size if enabled */
    if(webp === 'yes') images.push(await sharp(buffer).resize(width, height).webp({quality: imgQ}).toBuffer()
